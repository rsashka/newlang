...
//            "test_filed(arg1, arg2):_ := {$arg1.all(); $arg1.mul_($arg2);_;}\n"
//            ));
//
...
//
//    std::ofstream file("fsimple_test.temp.cpp");
//    file << sstr.str();
//    file.close();
//
//    std::string out;
//    int exit_code;
//    ASSERT_TRUE(NewLang::GccMakeModule("fsimple_test.temp.cpp", "fsimple_test.temp.nlm", nullptr, &out, &exit_code)) << exit_code << " " << out;
//
//    RuntimePtr opts = RunTime::Init();
//    Context ctx(opts);
//
//    Module module;
//    ASSERT_TRUE(opts->LoadModule("fsimple_test.temp.nlm"));
//
...
//
//
//    EXPECT_FALSE(test_and->Call(Object::Arg(123, "arg1"), Object::Arg(555, "arg2"))->GetValueAsBoolean());
//    EXPECT_TRUE(test_and->Call(Object::Arg(123, "arg1"), Object::Arg(123, "arg2"))->GetValueAsBoolean());
//    EXPECT_TRUE(test_and->Call(Object::Arg(555, "arg1"), Object::Arg(555, "arg2"))->GetValueAsBoolean());
//    EXPECT_FALSE(test_and->Call(Object::Arg(0, "arg1"), Object::Arg(0, "arg2"))->GetValueAsBoolean());
//
//    EXPECT_TRUE(test_or->Call(Object::Arg(123, "arg1"), Object::Arg(555, "arg2"))->GetValueAsBoolean());
//    EXPECT_TRUE(test_or->Call(Object::Arg(555, "arg1"), Object::Arg(555, "arg2"))->GetValueAsBoolean());
//    EXPECT_TRUE(test_or->Call(Object::Arg(123, "arg1"), Object::Arg(123, "arg2"))->GetValueAsBoolean());
//    EXPECT_TRUE(test_or->Call(Object::Arg(555, "arg1"), Object::Arg(0, "arg2"))->GetValueAsBoolean());
//    EXPECT_FALSE(test_or->Call(Object::Arg(0, "arg1"), Object::Arg(0, "arg2"))->GetValueAsBoolean());
//
//    EXPECT_TRUE(test_xor->Call(Object::Arg(123, "arg1"), Object::Arg(555, "arg2"))->GetValueAsBoolean());
//    EXPECT_FALSE(test_xor->Call(Object::Arg(555, "arg1"), Object::Arg(555, "arg2"))->GetValueAsBoolean());
//    EXPECT_FALSE(test_xor->Call(Object::Arg(123, "arg1"), Object::Arg(123, "arg2"))->GetValueAsBoolean());
//    EXPECT_TRUE(test_xor->Call(Object::Arg(0, "arg1"), Object::Arg(0, "arg2"))->GetValueAsBoolean());
//}

//TEST(NewLang, FunctionFollow) {
//    TermPtr func;
//    Parser parser(func);
//    parser.Parse("func(arg1, arg2 = 5) :- { [$arg1 < $2] -> %{ return $arg1; %} -> %{ return *$1 * *$2; %} };");
//    std::string to_str = func->toString();
//    TermPtr func2;
//    Parser parser2(func2);
//    parser2.Parse(to_str);
//    ASSERT_STREQ(to_str.c_str(), func2->toString().c_str());
//
//    ASSERT_EQ(TermID::TRANSPARENT, func->getTermID()) << newlang::toString(func->getTermID());
//    ASSERT_TRUE(func->Left());
//    ASSERT_TRUE(func->Right());
//
//    std::ostringstream sstr;
//    ASSERT_TRUE(NewLang::MakeCppFile(func, sstr)) << sstr.str();
//
//    std::ofstream file("follow.temp.cpp");
//    file << sstr.str();
//    file.close();
//
//    std::string out;
//    int exit_code;
//    ASSERT_TRUE(NewLang::GccMakeModule("follow.temp.cpp", "follow.temp.nlm", nullptr, &out, &exit_code)) << exit_code << " " << out;
//
//    Module module;
//    ASSERT_TRUE(module.Load("follow.temp.nlm"));
//
//    FunctionType * call = reinterpret_cast<FunctionType *> (dlsym(module.m_handle, MangleName("func").c_str()));
//    ASSERT_TRUE(func) << MangleName("func");
//
//
//    Object args1(Object::Arg(), Object::Arg(5, "arg1"), Object::Arg(5, "arg2"));
//    ObjPtr result = (*call)(nullptr, args1);
//    ASSERT_TRUE(*result == Object::CreateValue(25));
//
//    Object args2(Object::Arg(), Object::Arg(3, "arg1"), Object::Arg(10, "arg2"));
//    result = (*call)(nullptr, args2);
//    ASSERT_TRUE(*result == Object::CreateValue(30));
//
//    Object args3(Object::Arg(), Object::Arg(50, "arg1"), Object::Arg(10, "arg2"));
//    result = (*call)(nullptr, args3);
//    ASSERT_TRUE(*result == Object::CreateValue(50));
//
//
//    Context ctx(RunTime::Init());
//    ASSERT_TRUE(ctx.m_runtime->GetProcAddress("func", "follow.temp.nlm"));
//    ObjPtr follow = ctx.m_runtime->GetFunc("func");
//    ASSERT_TRUE(follow);
//
//    ASSERT_ANY_THROW(
...
//
//    ASSERT_ANY_THROW(
//            (*follow)(Object::Arg(5), Object::Arg(5), Object::Arg(5)) // Функция принимает один или два аргумента
//            );
//
...
//    TermPtr func;
//    Parser parser(func);
//    parser.Parse("func(arg1, count) :- {[$count==0]->{## \"Ошибка\";}; $arg1[].start:=$count(); $arg1[].stop:=$count(); [$count] <-> {$arg1[].step:=$count(); $count--; $arg1.stop=$count;}; $$ $1;};");
//
//    ASSERT_EQ(TermID::TRANSPARENT, func->getTermID()) << newlang::toString(func->getTermID());
//    ASSERT_TRUE(func->Left());
//    ASSERT_TRUE(func->Right());
...
//    ASSERT_TRUE(NewLang::MakeCppFile(func, sstr)) << sstr.str();
//
//    std::ofstream file("repeat.temp.cpp");
//    file << sstr.str();
//    file.close();
//
//    std::string out;
//    int exit_code;
//    ASSERT_TRUE(NewLang::GccMakeModule("repeat.temp.cpp", "repeat.temp.nlm", nullptr, &out, &exit_code)) << exit_code << " " << out;
//
//
//    Context ctx(RunTime::Init());
//    ASSERT_TRUE(ctx.m_runtime->LoadModule("repeat.temp.nlm"));
...
//
//    ASSERT_THROW(
//            (*repeat)(Object::Arg(5), Object::Arg(5), Object::Arg(5)), // Функция принимает два аргумента
//            std::invalid_argument
//            );
//
//    ASSERT_THROW(
//            (*repeat)(Object::Arg(5), Object::Arg(0)), // Второй аргумент не ноль!
//            newlang_exception
//            );
//
//    ObjPtr result = repeat->Call(Object::Arg(Object::CreateDict(), "arg1"), Object::Arg(1, "count"));
//    ASSERT_TRUE(result);
//    ASSERT_STREQ("(start=1, stop=0, step=1,)", result->toString().c_str());
//
//    result = repeat->Call(Object::Arg(Object::CreateDict()), Object::Arg(2));
//    ASSERT_TRUE(result);
//    ASSERT_STREQ("(start=2, stop=0, step=2, step=1,)", result->toString().c_str());
//
//    result = repeat->Call(Object::Arg(Object::CreateDict()), Object::Arg(3));
//    ASSERT_TRUE(result);
//    ASSERT_STREQ("(start=3, stop=0, step=3, step=2, step=1,)", result->toString().c_str());
//}
...
///*
// * Основонй тест соответствия между компиляцией и интерпретацией текста программы
// * Реализован как набор выражений, возвращающих результат.
// * В начале все выражения выпоняются интерпретатором, тестируются на корректность и собираются в массив.
// * Далее выражения записываюстя в исходный фай в виде модуля, который компилируется, загружается и выполняется.
...
//        {"Jake2=human(пол=\"муж.\", parent=[&Tom2, &Janna2,])", "Jake2 $= @human(м, [  &Tom2, &Janna2, ] )"},
//        {"Tim2=human(пол=\"муж.\", parent=[&Tom2,])", "Tim2$=@human(пол=м, parent=[&Tom2,])"},
//        //        {"brother={}", "brother(test1, test2) &&= $test1!=$test2, $test1.sex==м, intersec($test1.parent, $test2.parent)"},
//    };
...
//    // Оператор (символ +)
//    ASSERT_STREQ("300", opts->Eval(&ctx, "100+200")->GetValueAsString().c_str());
//    // Вызов встроенной функции
//    Args min_args(Object::CreateValue(200), Object::CreateValue(100), Object::CreateValue(300)); // min(200,100,300)
//    ASSERT_STREQ("100", Context::CallByName(&ctx, "min", min_args)->GetValueAsString().c_str());
//
//    ObjPtr min_ret = newlang_min(&ctx, Args::Arg(Object::CreateValue(200)), Args::Arg(Object::CreateValue(100)), Args::Arg(Object::CreateValue(300)));
//    ASSERT_TRUE(min_ret);
...
//    ASSERT_TRUE(opts->Eval(&ctx, "name2 $= 222; name3 $= 333")->GetValueAsString().c_str());
//
//    ASSERT_EQ(0, ctx.Current()->session.find("name2")->second->RefCount());
//    ASSERT_STREQ("222", opts->Eval(&ctx, "name2")->GetValueAsString().c_str());
//    ASSERT_EQ(0, ctx.Current()->session.find("name2")->second->RefCount());
//
//    ASSERT_STREQ("333", opts->Eval(&ctx, "name3")->GetValueAsString().c_str());
//
//
...
//            std::runtime_error);
//
//    ASSERT_STREQ("FIELD1", Object::GetField(field, "field1")->GetValueAsString().c_str());
//
//    Object::Ptr obj = Object::CreateDict(Args::Arg("f", field), Args::Arg("f2", field));
//
//    ASSERT_STREQ("FIELD1", Object::GetField(Object::GetField(obj, "f"), "field1")->GetValueAsString().c_str());
//    ASSERT_STREQ("FIELD2", Object::GetField(Object::GetField(obj, "f2"), "field2")->GetValueAsString().c_str());
//
//    ASSERT_STREQ("(val=456,)", opts->Eval(&ctx, "field $= (val=456,)")->GetValueAsString().c_str());
//    ASSERT_STREQ("(val=456,)", opts->Eval(&ctx, "field")->GetValueAsString().c_str());
//    ASSERT_STREQ("123", opts->Eval(&ctx, "field.name := 123")->GetValueAsString().c_str());
//    ASSERT_STREQ("(val=456, name=123,)", opts->Eval(&ctx, "field")->GetValueAsString().c_str());
//
//
//
//
//    ASSERT_EQ(1, opts->m_user_terms.size());
//    ASSERT_STREQ("test=@term(пол=, parent=[,])", opts->Eval(&ctx, "test   @=  term(пол=,    parent  =   [,]   )")->GetValueAsString().c_str());
...
//    ASSERT_EQ(0, ctx.Current()->session.find("name2")->second->RefCount());
//    ASSERT_STREQ("222", ctx.Current()->session.find("name2")->second->toString().c_str());
//    ASSERT_STREQ("Tom=test(пол=222, parent=[,])", opts->Eval(&ctx, "Tom $= @test(пол=name2)")->GetValueAsString().c_str());
//    ASSERT_EQ(0, ctx.Current()->session.find("name2")->second->RefCount());
//
//    ASSERT_STREQ("[&Tom,]", opts->Eval(&ctx, "ref$=[&Tom,]")->GetValueAsString().c_str());
//    ASSERT_STREQ("Tim=test(пол=, parent=[&Tom,])", opts->Eval(&ctx, "Tim $= @test(parent=[&Tom,])")->GetValueAsString().c_str());
//    ASSERT_STREQ("brother={}", opts->Eval(&ctx, "brother(test1, test2) &&= $test1!=$test2, $test1.sex==м, intersec($test1.parent, $test2.parent)")->GetValueAsString().c_str());
//
//
//    std::string str = "#!./dist/Debug/GNU-Linux/nlc --exec\n"
//            "// Test source code for verify consistency between compilation and evaluation modes\n"
//            "\n";
//
//    for (auto item : eval_items) {
//        ASSERT_STREQ(item.result, opts->Eval(&ctx, item.eval)->GetValueAsString().c_str());
//        str += item.eval;
//        str += ";\n";
//    }
//
...
//    str += "\n$$ := consistency;\n";
//
//    std::ofstream out("consistency.temp.sh");
//    out << str;
//    out.close();
//
//
//    Context ctx_module(NewLang::Init());
//    ObjPtr result_module = NewLang::ExecModule("consistency.temp.sh", "consistency.temp.nlm", false, &ctx_module);
...
//        ASSERT_TRUE(res);
//        ASSERT_STREQ(eval_items[i].result, res->GetValueAsString().c_str()) << eval_items[i].result << " != " << res->GetValueAsString();
//    }
//
...
//        ASSERT_TRUE(res);
//        ASSERT_STREQ(array_items[i].result, res->GetValueAsString().c_str()) << array_items[i].result << " != " << res->GetValueAsString();
//
...
//    ASSERT_TRUE(e);
//    JIT_PRINT jit_print = reinterpret_cast<JIT_PRINT> (opts->m_jit->GetFunction("jit_print"));
//    ASSERT_TRUE(jit_print);
//    ASSERT_EQ(0, jit_print(""));
//    ASSERT_EQ(9, jit_print("jit_print"));
//
//
//    //    std::ofstream out("native.temp.nlp");
//    //    out << "native(str=\"default arg\") $= { %{ $$=$str; printf(\"%s\", static_cast<char *>($str)); %} };";
//    //    out.close();
//    //
//    //    ASSERT_TRUE(opts->CompileFunctions("native(str=\"default arg\") $= { %{ $$=$str; printf(\"%s\", static_cast<char *>($str)); %} };"));
//    //    ASSERT_TRUE(opts->CompileModule("native.temp.nlp", "native.temp.nlm"));
//    //    ASSERT_TRUE(opts->LoadModule("native.temp.nlm"));
//    //
...
TEST(NewLang, DISABLED_Function) {
    ...
    std::ofstream file("function_test.temp.cpp");
    file << sstr.str();
    file.close();
}
...
#define FUNC_RES "func_res(arg1: Char, arg2: Int): Integer := { $arg2+=$arg1; };"
    ...
    std::ofstream file("call_types.temp.cpp");
    file << sstr.str();
    file.close();

    std::string out;
    int exit_code;
    ASSERT_TRUE(NewLang::GccMakeModule("call_types.temp.cpp", "call_types.temp.nlm", nullptr, &out, &exit_code)) << exit_code << " " << out;
    ...
    Object args;
    ASSERT_TRUE(ctx.m_runtime->m_modules["call_types.temp.nlm"]);
    ASSERT_NO_THROW(ctx.m_runtime->m_modules["call_types.temp.nlm"]->Main(&ctx, args));
    //@todo Контроль переполнения при операциях для типизированных переменных ????????????????
    //@todo Такой же как и для остальных операций !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    //ASSERT_THROW(opts->m_modules["call_types.temp.nlm"]->Main(&ctx, args), newlang_exception);
