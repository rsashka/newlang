...
TEST(LLVM, JIT) {
    ...
    ASSERT_TRUE(RunTime::LLVMInitialize());
...
TEST(LLVM, Include_h_i_JIT) {
    ...
    std::string include_test_cpp = ReadFile("../src/test/include_test.cpp");
    ASSERT_TRUE(include_test_cpp.size() > 1000);


    std::string opt_str = ReadFile("../src/build_options.txt");
    ASSERT_TRUE(opt_str.size() > 100);

    std::vector<std::string> opts = Macro::SplitChar(opt_str, " \t\r\n");

    opts.push_back("-I../src");
