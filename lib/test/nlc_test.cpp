...
//
//    ASSERT_STREQ("", ReplaceFileExt("", ".src", ".nlm").c_str());
//    ASSERT_STREQ(".", ReplaceFileExt(".", ".src", ".nlm").c_str());
//    ASSERT_STREQ("name.nlm", ReplaceFileExt("name", ".src", ".nlm").c_str());
//    ASSERT_STREQ("name.nlm", ReplaceFileExt("name.src", ".src", ".nlm").c_str());
//    ASSERT_STREQ("name.test.nlm", ReplaceFileExt("name.test", ".src", ".nlm").c_str());
//    ASSERT_STREQ("name.", ReplaceFileExt("name.", ".src", ".nlm").c_str());
//}
//
//TEST(NLC, Eval) {
//    NLC nlc6("path  --badarg --output=output_file");
...
////    мир!\\n\");\n"; out.close();
////
////    ASSERT_TRUE(opts->CompileModule("temp/native.temp.src", "temp/native.temp.nlm"));
...
////
////
////    std::ofstream out2("temp/native.temp.src");
////    out2 << "#!/bin/nlc\n";
////    out2 << "native(str=\"second\") := { %{ printf(\"%s\", static_cast<char
////    *>(*$str)); return $str; %} };\n"; out2 << "//\\native(\"Привет,
////    мир!\\n\");\n"; out2.close();
////
////    NLC compile2("/bin/nlc --compile=temp/native.temp.src");
////    ASSERT_STREQ("native.temp.src", compile2.m_ifile.c_str());
