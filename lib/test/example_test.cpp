...
TEST(Example, Rational) {
    ...
    ObjPtr test_printf;
    ASSERT_NO_THROW(test_printf = jit->Run("test_printf(format:FmtChar, ...):Int32 := %printf..."));
    ...
    ASSERT_NO_THROW(frac_test = jit->Run("test_arg(1)"))
            << jit->m_main_ast->m_int_vars.Dump() << "\n"
...
TEST(Example, Tensor) {
    ...
    TermPtr ast = jit->GetParser()->Parse("print('%%%s\\n', :StrChar(123))");
    ASSERT_TRUE(ast);
    ...
    ast = jit->GetParser()->Parse("var := 123; var2 := var * 123");
    ASSERT_TRUE(ast);
    ...
    ast = jit->GetParser()->Parse("rand():Int32 := %rand ...;");
    ASSERT_TRUE(ast);
    ...
    ast = jit->GetParser()->Parse("r():Int32 := %rand ...; :Tensor[10](42, ... r() ...)");
    ASSERT_TRUE(ast);
    ...
    ast = jit->GetParser()->Parse("sss := :Tensor[10](42, ... rnda() ...)");
    ASSERT_TRUE(ast);
