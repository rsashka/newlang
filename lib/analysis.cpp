...
bool AstAnalysis::CheckError(bool result) {
    if (!result) {
        ...
        if (m_diag->m_error_limit && m_diag->m_error_count >= m_diag->m_error_limit) {
            ...
            LOG_PARSER("fatal error: too many errors emitted %d, stopping now [-nlc-error-limit=]", m_diag->m_error_count);
        }
    }
    return result;
}
...
std::string AstAnalysis::MakeInclude(const TermPtr & ast) {
    ...
    if (ast->isExport()) {
        if (ast->m_id == TermID::MACRO_DEL || ast->m_left->m_id == TermID::MACRO_SEQ) {
            result += ast->toString(true);
            result += ";\n";
        } else {
            ASSERT(ast->isCreate());
            TermPtr term = ast->m_left;
            while (term) {
                if (Term::isExportName(term)) {
        ...
        result += MakeInclude(ast->m_right);
    }
    for (auto &elem : ast->m_follow) {
        result += MakeInclude(elem);
    }
