...
std::string MacroBuffer::toHash(const TermPtr &term) {
    //    if(term->m_id == TermID::MACRO || term->m_id == TermID::MACRO_SEQ || term->m_id == TermID::MACRO_DEL) {
    if(term->m_id == TermID::MACRO_SEQ || term->m_id == TermID::MACRO_DEL) {
        if(!term->GetMacroId().empty()) {
...
TermPtr MacroBuffer::EvalOpMacros(const TermPtr & term) {
    ...
    TermPtr macro = GetMacroById(term->Left()->GetMacroId());
    ...
    if(macro) {
        ...
        for (auto iter = found->second.begin(); found != end() && iter != found->second.end(); ++iter) {
            if(MacroBuffer::IdentityMacro(term->Left()->GetMacroId(), *iter) || MacroBuffer::IdentityMacro(iter->get()->GetMacroId(), term->Left())) {
        ...
        macro->m_right = CreateMacroFromOp(term)->Right();
...
bool MacroBuffer::IdentityMacro(const BlockType &buffer, const TermPtr & macro) {
    ...
    if(!macro) {// || buffer.size() < macro->m_macro_id.size()) {
        return false;
    }
...
size_t MacroBuffer::ExtractArgs(BlockType &buffer, const TermPtr &term, MacroArgsType & args) {
    ...
    if(!((term->getTermID() == TermID::MACRO || term->getTermID() == TermID::MACRO_SEQ
            || term->getTermID() == TermID::MACRO_STR || term->getTermID() == TermID::MACRO_DEL))) {
...
BlockType MacroBuffer::ExpandMacros(const TermPtr &macro, MacroArgsType & args) {
    ...
    BlockType result;
    BlockType seq = macro->Right()->m_follow;
...
next_escape_token:
    ...
    while(lexer->m_macro_count == 0 && !m_macro_analisys_buff.empty()) {
        ...
        if(m_macro) {
            ...
            if(!(m_macro_analisys_buff[0]->getTermID() == TermID::NAME || m_macro_analisys_buff[0]->getTermID() == TermID::MACRO)) {
                break;
            }
            ...
            if(macro_done) {
                ...
                ASSERT(m_macro_analisys_buff.size() >= macro_done->m_follow.size());
        ...
        *yylloc = m_macro_analisys_buff.at(0)->m_lexer_loc;
        ...
        if(m_postlex) {
            ...
            if((*yylval)->getTermID() == TermID::MACRO_SEQ || (*yylval)->getTermID() == TermID::MACRO_DEL) {
                for (int i = 0; i < (*yylval)->m_follow.size(); i++) {
                    m_postlex->push_back((*yylval)->m_follow[i]->m_text);
                }
                m_postlex->push_back((*yylval)->m_text);
            }
