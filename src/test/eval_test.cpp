...
TEST(Eval, Tensor) {
    ...
    ASSERT_NO_THROW(ddd = rt->Run(":Tensor( (1,2,3,) )"));
    ASSERT_TRUE(ddd);
    ...
    ASSERT_NO_THROW(tt = rt->Run(":Single[3,2]( 42, ... rand() ... )"));
...
TEST(Eval, TypesNative) {
    ...
    ObjPtr fopen2 = rt->CreateNative("fopen2 ::= :Pointer('fopen(filename:StrChar, modes:StrChar):File')");
    ASSERT_TRUE(fopen2);
