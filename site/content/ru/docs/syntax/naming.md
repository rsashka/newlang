...
Перегрузка [функций](/ru/docs/types/funcs/) по типам аргументов в *NewLang* отсуствует, 
поэтому несколько функций с одинаковым именем, но разными аргументами определить нельзя,
но можно переопределить функцию с расширением [типов](/ru/docs/types/generics/) аргументов и/или увеличив их количество.
...
а тип создаваемого объекта будет [зависеть](/ru/docs/ops/create/) от его месторасположения:
...
- '**::**' — двойное двоеточие является разделителем [пространства имен](/ru/docs/syntax/namespace/),
        ...
        а значение объекта сохраняется даже после завершения текущей области видимости.
        Если имя начинается на '**::**', то область видимости объекта будет глобальной и он будет доступен в рамках всего приложения.
        В другом случае, например '**@::**' - используется имя статического объекта в текущем пространстве имен, 
        а его видимость будет ограничена только текущим [программным модулем](/ru/docs/syntax/modules/).
...
- '**^**' — Все объекы по умолчантию не изменеяемые. Символ *карет* (крышечка/домик) после имени при создании объекта,
        используется для придания ему мутабельности (возможности изменять свое значение). 
        Мутабельные аргументы функций могут быть только ссылками.
...
[Создание объектов происходит с помощью специальных операторов по определенным правилам](/ru/docs/ops/create/).

В других же случаях, когда *NewLang* встречает *простое* имя объекта без квалификатора ([сигила](https://en.wikipedia.org/wiki/Sigil_(computer_programming))), 
...
с его декларацией или конкретным созданным объектом по его [внутреннему имени](/ru/docs/arch/mangling/).
...
- в последнюю очередь выполняется поиск среди глобальных объектов с постепенным расширением пространства имен области поиска от текущей до глобальной
...
http://rsdn.org/article/submit/newcpp/newcpp.xml
...
> Cтандарт С++11 объявляет (3.10) пять терминов: lvalue, rvalue, xvalue, а также glvalue (обобщение lvalue и xvalue) 
> и prvalue (более узкая специализация rvalue). 
> Базовыми терминами являются знакомые нам lvalue, rvalue и новый термин xvalue (от expiring value). 
> Забегая вперёд – например, xvalue является результатом функции, возвращающей rvalue ссылку. 
> То, что понималось под rvalue в C++98, в C++11 стало prvalue (в то время как rvalue обобщает xvalue и prvalue), 
> а новый термин glvalue обобщает lvalue и xvalue. 
>
> Можно (неточно, но образно) сказать, что prvalue – это то, брать адрес от чего нельзя, lvalue – от чего можно, а xvalue – от чего бесполезно.
