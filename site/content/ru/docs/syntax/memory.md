...
где каждое значение в памяти может иметь контролируемое количество переменных с *сильным* указателем. 

И когда переменная с *сильным* указателем уходит из области видимости, счетчик ссылок уменьшается, а при достжении нуля память объекта освобождается. 
Фактически, это автоматическое управление памятью с помощью подсчёта ссылок на этапе компиляции без использования сборщика мусора.

Под *"полным контролем ссылок"* имеется ввиду, что возможность получения ссылки на переменную 
должно быть декларировано в явном виде еще при создании переменной.

Существование ссылок на объекты предполагает, в том числе, и возможность 
одновременного доступа к данным из нескольких потоков.
Из-за этого управление памятью и контроль ссылок включают в себя элементы межпотокового взаимодействия, 
так как совместное владение ссылками по любому требует механизмов синхронизации доступа к разеделяемой памяти объектов. 

При определении переменной должно быть указано, какие типы ссылок допускается создавать,
и какая модель контроля совместного доступа должна быть использована.

Весь механизм подсчета ссылок и проверки их корректности определяется правилам и реализован на **уровне синтаксиса**.
...
- Переменная - владаец объекта, это **корневая** *не контролируемая* переменную с *сильной* ссылкой.
- Каждый объект может иметь только одну **корневую** *не контролируемую* переменную с *сильной* ссылкой
