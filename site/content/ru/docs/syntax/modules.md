...
*Имя модуля может содержать только строчные английские буквы, цифры или символ подчерка (кроме первой и последеней позиции).
...
Под модулем в *NewLang* понимается **файл** с исходным кодом (с расширением *.src). 
...
Все объекты, определенные внутри одного модуля без указания глобального простанства имен, 
видны только в рамках текущего файла и из других модулей, подключенных после её определения.

Время жизни статических и локальных переменных самого верхнего уровня модуля одинаковые и ограничены временем жизни самого модуля,
...
Относительное имя программного модуля начинается на один символ "**\\**", 
...
(или указанного в списке каталогов для поиска программных модулей, 
...
Если перед именем импортируемого молуя стоит символ препроцессора, т.е. имя модуля начинается на '**@\\**' или '**@\\\\**',
это означает, что модуль импортирует не только код, но и все препроцессорые макросы.
...
При динамической линковке модуля, макросы импортировать невозможно.
