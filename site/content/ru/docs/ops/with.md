...
### Захват легких ссылок совместного доступа {#light-link}
...
Чтобы можно было использовать легкие ссылки на объекты в других потоках, для них требуется создать легкие ссылки для совместного доступа ("**&?**").
...
```python
    ...
    multi_thread_func( &? win1, &? win2) := {

        && @::sync ::= _; # Static synchronization object
...
### Расширенный синтаксис определения ссылок {#extended-link}
...
### Автоматическое освобождение ресурсов {#with-example}
