...
По умолчанию все методы класса являются виртуальными и могут быть переопределены в классах наследниках.
Чтобы запретить возможность переопределения метода, он должен быть указан как [иммутабельный](/ru/docs/syntax/naming/#immutable).
Если виртуальный метод класса переопределяется как с признаком иммутабельности, это равносильно определению метода *override final*.
