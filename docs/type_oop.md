...
# Синоним класса
:Alter := :Name;
:Alter::Alter() := { # Альтернативный конструтор взамен Name() _Name() и __Name()
# Но поля таким образом изменить нельзя, только методы!!!! 
}

:Name2 := :Name() {
    Name2() := { # Конструктор после Name    
        $0;
    };
};
