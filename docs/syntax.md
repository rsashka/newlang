...
**:None** (пусто) — не содержит значения (точнее имеет одно значение None) и совместим с любым другим типом данных. Указывается в тексте программы как один подчерк «_». Значение None имеют не инициализированные переменные и при попытке чтения из такой переменной возникает ошибка.
...
Так как тензоры могту иметь больше одного значения, то и в качестве аргументов можно передавать более одного значения, а итоговым результатом будет тензор содержащий указанные значения с автоматическим преобразованим переданных значений к требуемому типу.
...
Поэтому, подход к обработке исключений следующий:

Программный код, который может привести к ошибке, заключается в двойные фигурные скобки *()\{\{ любой код или вызов одиночной функции \}\}*, а результат выполнения такого блока кода присваивается переменной. После этого анализируется возвращенное значение, например, оператором сравнения по образцу:

```
$result := (){ {     
	$value := call_or_exception1();   # начало аналога блока try
    [условие1] -> { -- :Error -- };
    [условие2] -> { -- $value -- };
	$value := call_or_exception2();   # конец аналога блока try
} };                 
